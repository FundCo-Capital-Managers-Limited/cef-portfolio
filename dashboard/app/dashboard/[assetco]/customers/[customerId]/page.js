...
export default async function CustomerDetailPage({ params }) {
  ...
  return (
    <div className="space-y-8">
      <div>
        <BackLink href={`/dashboard/${params.assetco}`}>{params.assetco} Dashboard</BackLink>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-4 text-sm space-y-2">
            ...
            <div className="flex justify-between"><span className="text-gray-500">Segment</span><span>{customer.customer_segment || 'N/A'}</span></div>
            ...
            <div className="flex justify-between"><span className="text-gray-500">Location</span><span>{[customer.location_lga, customer.location_state].filter(Boolean).join(', ') || 'N/A'}</span></div>
            <div className="flex justify-between"><span className="text-gray-500">Contract signed</span><span>{customer.contract_signed_date || 'N/A'}</span></div>
            <div className="flex justify-between"><span className="text-gray-500">Expected installation</span><span>{customer.expected_installation_date || 'N/A'}</span></div>
            <div className="flex justify-between"><span className="text-gray-500">Expected monthly payment</span><span>{customer.expected_monthly_payment_ngn ? formatCurrency(customer.expected_monthly_payment_ngn) : 'N/A'}</span></div>
            <div className="flex justify-between"><span className="text-gray-500">Contract term</span><span>{customer.contract_term_months ? `${customer.contract_term_months} months` : 'N/A'}</span></div>
